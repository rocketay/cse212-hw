...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                ...
                // If the node to remove is the head (but not the tail)
                else if (curr == _head)
                {
                    _head = curr.Next;
                    _head!.Prev = null; // Disconnect the new head from the old head
                }
                // If the node to remove is the tail (but not the head)
                else if (curr == _tail)
                {
                    _tail = curr.Prev;
    ...
    {
        // Start at the tail and iterate backward
        var curr = _tail;
        while (curr is not null)
        {
            yield return curr.Data; // Provide (yield) each item to the user
